...
static uint32_t spi_write_then_read(void *txbuf, uint32_t txlen, void *rxbuf,
		uint32_t rxlen);
...
static void spi_flash_hw_init(void)
{
	...
	gpio_init(GPIOA, PIN0 | PIN1 | PIN2 | PIN3, GPIO_MODE_AF6, GPIO_PULL_NONE,
			GPIO_DRV_3);
...
void spi_write(uint8_t *data, uint32_t cnt)
{
